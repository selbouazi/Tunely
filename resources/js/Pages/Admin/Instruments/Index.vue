...
const applyDiscount = () => {
    form.porcentaje = discountPercent.value;
    form.post(route('admin.instrumentos.bulk-discount'));
...
<template>
    <AdminLayout>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
            ...
            <Link :href="route('admin.instrumentos.create')" class="bg-[#E87F24] text-white px-4 py-2 rounded hover:bg-[#FFC81E] whitespace-nowrap">
                + Nuevo producto
            </Link>
            <button @click="showDiscountModal = true" class="border border-[#E87F24] text-[#E87F24] px-4 py-2 rounded hover:bg-[#E87F24] hover:text-white whitespace-nowrap text-sm">
                Descuento global
            </button>
        </div>

        <div class="flex flex-wrap gap-3 mb-4">
            <select v-model="filterCat" class="border border-gray-300 px-3 py-2 rounded text-sm bg-white">
                <option value="todas">Todas las categorías</option>
                <option v-for="cat in categories" :key="cat.id" :value="cat.id">{{ cat.nombre }}</option>
            </select>
            <label class="flex items-center gap-2 text-sm text-gray-600">
                <input type="checkbox" v-model="sortStock" class="rounded">
            ...
            <table class="w-full text-sm">
                ...
                <tbody>
                    <tr v-for="v in filtered" :key="v.id" class="border-t hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-center">
                            <span v-if="v.disponible && v.stock > 0" class="text-green-600 text-xs bg-green-50 px-2 py-1 rounded">Disponible</span>
                            <span v-else class="text-red-600 text-xs bg-red-50 px-2 py-1 rounded">Agotado</span>
                        </td>
                        <td class="px-4 py-3 text-right space-x-2 whitespace-nowrap">
                            <Link :href="route('admin.instrumentos.edit', v.id)" class="text-[#E87F24] hover:underline text-sm">Editar</Link>
                            <button
        ...
        <Modal :show="showDiscountModal" @close="showDiscountModal = false" max-width="md">
            <div class="p-6">
                ...
                <div class="flex justify-end gap-3 mt-6">
                    ...
                    <button @click="applyDiscount" class="px-4 py-2 text-sm text-white bg-[#E87F24] rounded hover:bg-[#FFC81E]">
                        Aplicar descuento
                    </button>
                </div>
            </div>
